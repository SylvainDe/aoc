...
def least_gold_to_win(boss):
    heap = [(0, 0, 0, False, False, ())] # Money spent, damage, armor, has weapon, has armor, rings
    while heap:
        money, damage, armor, has_weapon, has_armor, rings_tuple = heapq.heappop(heap)
        if win_fight(Player(100, damage, armor), boss):
            return money
        for e, has_weapon2, has_armor2, rings_tuple2 in get_possible_states(has_weapon, has_armor, rings_tuple):
            heapq.heappush(heap, (money+e.cost, damage+e.damage, armor+e.armor, has_weapon2, has_armor2, rings_tuple2))
    return None
