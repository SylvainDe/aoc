...
def get_nb_arrangements(chars, groups):
    ...
    min_blocks = sum(groups) + len(groups) - 1
    ...
    return get_nb_arrangements(FULL + other_c, groups) + get_nb_arrangements(EMPTY + other_c, groups)


def get_sum_nb_arrangement(records):
    return sum(get_nb_arrangements(chars, groups) for chars, groups in records)
