...
class YearData:
    def __init__(self, year):
        ...
        try:
            ...
            stats = dict()
        days = [
            DayData(year, day, stats.get(day, dict()))
...
class DayData:
    def __init__(self, year, day, stats):
        ...
        assert (
            (self.part1_time is None)
            == (self.part1_rank is None)
            == (self.part1_score is None)
        )
        assert (
            (self.part2_time is None)
            == (self.part2_rank is None)
            == (self.part2_score is None)
        )
        self.nb_stars = (self.part1_time is not None) + (self.part2_time is not None)

    def get_pretty_title(self):
        return (
            self.title if self.title is not None else self.format_str("{year}/12/{day}")
        )
...
if __name__ == "__main__":
    ...
    with open(destfile, "w") as f:
                ...
                f.write(
                    format_table_colums(
                        [
                            ...
                            "{} {}".format(
                                format_file(d.puzzle_file), format_file(d.input_file)
                            ),
                            "*" * d.nb_stars,
        ...
        columns = (
            ["Day"] + [y.event_url + " " + y.github_url for y in years] + ["Total"]
        )
        f.write(format_table_colums(columns))
        f.write(format_table_colums(("---" for _ in columns)))
